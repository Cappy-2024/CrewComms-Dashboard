...
const COMMANDS = ['sessionstart', 'sessionend', 'sessioncancel', 'editpoints', 'leaderboard', 'rank'];
...
// "Is the bot in this guild?" is re-checked on every single request but
// almost never changes, so a short TTL removes one Discord round trip from
// each of the dashboard's /roles, /settings and /points calls.
const botGuildCache = new Map<string, number>(); // guildId -> expires

async function assertBotInGuild(guildId: string) {
  const cached = botGuildCache.get(guildId);
  if (cached && cached > Date.now()) return;

  await discordFetch(`/guilds/${guildId}`, BOT_TOKEN, true);
  botGuildCache.set(guildId, Date.now() + 60_000);
}

// Usernames for the points table. Fetching the guild's member list is one
// request that covers everyone still in the server, versus one request per
// user_points row (up to 500) the way this used to work.
const guildMembersCache = new Map<string, { data: Map<string, { username: string; avatar: string | null }>; expires: number }>();
const userCache = new Map<string, { username: string; avatar: string | null } | null>();

async function getGuildMemberIndex(guildId: string) {
  const cached = guildMembersCache.get(guildId);
  if (cached && cached.expires > Date.now()) return cached.data;

  const index = new Map<string, { username: string; avatar: string | null }>();
  try {
    // Requires the GUILD_MEMBERS privileged intent, which the bot already
    // uses for role assignment.
    const members = await discordFetch(`/guilds/${guildId}/members?limit=1000`, BOT_TOKEN, true);
    for (const m of members as any[]) {
      if (m?.user?.id) index.set(m.user.id, { username: m.user.username, avatar: m.user.avatar ?? null });
    }
  } catch (err) {
    // Not fatal — callers fall back to per-user lookups for anyone missing.
    console.error('Guild member list unavailable, falling back to per-user lookups:', err);
  }

  guildMembersCache.set(guildId, { data: index, expires: Date.now() + 60_000 });
  return index;
}

/** Runs an async mapper over items a few at a time instead of all at once,
 *  so a large fallback batch can't burst straight into a Discord 429. */
...
async function lookupUser(userId: string) {
  ...
  try {
    const user = await discordFetch(`/users/${userId}`, BOT_TOKEN, true);
    const entry = { username: user.username, avatar: user.avatar ?? null };
...
async function requireGuildAdmin(req: Request, guildId: string) {
  ...
  try {
    await assertBotInGuild(guildId);
  } catch {
...
async function handleGetPoints(req: Request, guildId: string) {
  ...
  const rows = data || [];
  const memberIndex = await getGuildMemberIndex(guildId);

  // Anyone still in the server is already covered by the single member-list
  // request above; only stragglers (users who have since left) need an
  // individual lookup, and those are throttled and cached.
  const missing = rows.filter(row => !memberIndex.has(row.user_id)).map(row => row.user_id);
  const resolved = await mapWithConcurrency(missing, 5, lookupUser);
  missing.forEach((userId, i) => {
    const entry = resolved[i];
    if (entry) memberIndex.set(userId, entry);
  });

  const withNames = rows.map(row => {
    const entry = memberIndex.get(row.user_id);
    return { ...row, username: entry?.username ?? null, avatar: entry?.avatar ?? null };
  });
